...
{ 
	...
	isFinish = (_sprite->GetCurrentFrame() == 3 && level == 0) + (_sprite->GetCurrentFrame() == 7 && level == 1) + (_sprite->GetCurrentFrame() == 11 && level == 2);
	...
	if (StartFrame <= _sprite->GetCurrentFrame() && _sprite->GetCurrentFrame() < EndFrame)
		_sprite->Update(dt);
	else
	{
		_sprite->SelectFrame(StartFrame);
	}
...
void MorningStar::Render(Camera * camera)
{
	...
	{
		if (level == 0 && _sprite->GetCurrentFrame() == MORNINGSTAR_ANI_LEVEL0_START || _sprite->GetCurrentFrame() == MORNINGSTAR_ANI_LEVEL0_START + 1)
			return; // frame đầu và frame chuẩn bị đánh thì vẽ BBOX

		if (level == 1 && _sprite->GetCurrentFrame() == MORNINGSTAR_ANI_LEVEL1_START || _sprite->GetCurrentFrame() == MORNINGSTAR_ANI_LEVEL1_START + 1)
			return;

		if (level == 2 && _sprite->GetCurrentFrame() == MORNINGSTAR_ANI_LEVEL2_START || _sprite->GetCurrentFrame() == MORNINGSTAR_ANI_LEVEL2_START + 1)
			return; 
...
void MorningStar::Create(float simonX, float simonY, int simonDirection)
{
	...
	{
	case 0:
		_sprite->SelectFrame(MORNINGSTAR_ANI_LEVEL0_START); 
		_sprite->ResetTime();
		break;
	case 1:
		_sprite->SelectFrame(MORNINGSTAR_ANI_LEVEL1_START); 
		_sprite->ResetTime();

		break;
	case 2:
		_sprite->SelectFrame(MORNINGSTAR_ANI_LEVEL2_START);
		_sprite->ResetTime();
...
void MorningStar::UpgradeLevel()
{
	...
	{
		_sprite->SelectFrame(_sprite->GetCurrentFrame() + 4);
